...
function spectrum_validation_20151109_Tinkering()
clc
close all
        ...
        if strcmp(class(names),'char')
            ...
            if ~exist([path,filename,'.xml'])
                ...
                try
                    ...
                    print_code_now('Error validating spectra, please reset')
...
%     function data = validate_spectra()
    function validate_spectra()
        ...
        for i = 1:length(mods)
            ...
            elseif ~isempty(strfind(mods{i}, 'iTRAQ')) || ~isempty(strfind(mods{i}, 'TMT'))
                ...
                elseif strcmp(mods{i},'iTRAQ4plex (K)') || strcmp(mods{i},'iTRAQ4plex (N-term)')
                    iTRAQType = {'Fixed', 4};
                    iTRAQ_masses = [114.1106798,...                        116.1110695,...
                        117.1144243];
        ...
        [a,b] = system([msconvert_full_path, ' ', [RAW_path,RAW_filename] , ' -o ', RAW_path, ' --outfile ', filename, ' --mzXML -c config.txt']);
        ...
        tic
        [a,b] = system([msconvert_full_path, ' ', [RAW_path,RAW_filename] , ' -o ', RAW_path, ' --outfile ', filename, ' --text -c config.txt']);
...
% Write XLS file with iTRAQ data for scans in list
    function iTRAQ_to_Excel(excel_list)
        ...
        for i = 1:length(excel_list)
            ...
            line = [num2str(data{scan}.scan_number), '\t', data{scan}.protein, '\t', data{scan}.gi, '\t', data{scan}.fragments{id}.seq, '\t', num2str(round(data{scan}.pep_score)), '\t'];
