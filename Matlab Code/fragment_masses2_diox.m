...
function fragments = fragment_masses2_diox(seq, charge_state, prec_only)
global iTRAQ_masses iTRAQ_names c_term n_term;

global K k I L l M m z F T t W V R r H A N D C c E Q G P S s Y y;

global NH3 H2O H3PO4 HPO3 CO2 SOCH4 SO2CH4;
...
for i = 1:length(seq)
    ...
    if prec_only == 0
        ...
        [out_mass, out_names] = b_losses(charge_state, mass_fragments, {'NH_3', 'H_2O', 'H_3PO_4', 'SOCH_4', 'SO_2CH_4'}, [NH3; H2O; H3PO4; SOCH4; SO2CH4], [ones(1,length(mass_fragments)); ones(1,length(mass_fragments)); loc_st'; loc_m'; loc_n']);
        fragments{i}.all = [fragments{i}.all, out_mass];
        fragments{i}.all_names = [fragments{i}.all_names, out_names];
        
        [out_mass, out_names] = b_losses(charge_state, mass_fragments, {'NH_3', 'H_2O', 'HPO_3', 'SOCH_4', 'SO_2CH_4'}, [NH3; H2O; HPO3; SOCH4; SO2CH4], [ones(1,length(mass_fragments)); ones(1,length(mass_fragments)); loc_y'; loc_m'; loc_n']);
        fragments{i}.all = [fragments{i}.all, out_mass];
        fragments{i}.all_names = [fragments{i}.all_names, out_names];
        
        [out_mass, out_names] = b_losses(charge_state, mass_fragments, {'NH_3', 'H_2O', 'HPO_3-H_2O', 'SOCH_4', 'SO_2CH_4'}, [NH3; H2O; H3PO4; SOCH4; SO2CH4], [ones(1,length(mass_fragments)); ones(1,length(mass_fragments)); loc_y'; loc_m'; loc_n']);
        fragments{i}.all = [fragments{i}.all, out_mass];
        fragments{i}.all_names = [fragments{i}.all_names, out_names];
...
%         mass_fragments(end) = mass_fragments(end);
        ...
        loc_n = find_loc(seq{i}(end:-1:1),'n');
        loc_st = find_loc(seq{i}(end:-1:1),'[st]');
        loc_y = find_loc(seq{i}(end:-1:1),'y');
        
        [out_mass, out_names] = y_losses(charge_state, mass_fragments, {'NH_3', 'H_2O', 'H_3PO_4', 'SOCH_4', 'SO_2CH_4'}, [NH3; H2O; H3PO4; SOCH4; SO2CH4], [ones(1,length(mass_fragments)); ones(1,length(mass_fragments)); loc_st'; loc_m'; loc_n']);
        fragments{i}.all = [fragments{i}.all, out_mass];
        fragments{i}.all_names = [fragments{i}.all_names, out_names];
        
        [out_mass, out_names] = y_losses(charge_state, mass_fragments, {'NH_3', 'H_2O', 'HPO_3', 'SOCH_4', 'SO_2CH_4'}, [NH3; H2O; HPO3; SOCH4; SO2CH4], [ones(1,length(mass_fragments)); ones(1,length(mass_fragments)); loc_y'; loc_m'; loc_n']);
        fragments{i}.all = [fragments{i}.all, out_mass];
        fragments{i}.all_names = [fragments{i}.all_names, out_names];
        
        [out_mass, out_names] = y_losses(charge_state, mass_fragments, {'NH_3', 'H_2O', 'HPO_3-H_2O', 'SOCH_4', 'SO_2CH_4'}, [NH3; H2O; H3PO4; SOCH4; SO2CH4], [ones(1,length(mass_fragments)); ones(1,length(mass_fragments)); loc_y'; loc_m'; loc_n']);
        fragments{i}.all = [fragments{i}.all, out_mass];
        fragments{i}.all_names = [fragments{i}.all_names, out_names];
        ...
        % Internal Fragments
        [if_names, if_masses] = internal_fragments_diox(seq{i});
