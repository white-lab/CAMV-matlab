% Equivalent to plotassignfix_01222015 program in beta
% Changed my file location to input\ or output\
% Requires ProteoWizard to be packaged with this file

function spectrum_validation_20150122()
clc
close all
...
msconvert_full_path = 'ProteoWizard\"ProteoWizard3.0.6965"\msconvert';
                        ...
                        if ~isfield(temp.data{i},'code') && isfield(new_data{i},'code')
                            ...
                            if min(size(poss_seq)) > 0
                                ...
                                for j = 1:max(size(fragments))
                                    fragments{j}.validated = compare_spectra3(fragments{j}, new_data{i}.scan_data(temp_peaks,:), CID_tol);
                        ...
                        for j = 1:max(size(fragments))
                            if isfield(data{i}, 'scan_type')
                                if strcmp(data{i}.scan_type, 'HCD')
                                    fragments{j}.validated = compare_spectra3(fragments{j}, data{i}.scan_data(temp,:), HCD_tol);
                                else
                                    fragments{j}.validated = compare_spectra3(fragments{j}, data{i}.scan_data(temp,:), CID_tol);
                                end
                            else
                                fragments{j}.validated = compare_spectra3(fragments{j}, data{i}.scan_data(temp,:), CID_tol);
                            end
            ...
            for j = 1:max(size(fragments))
                if isfield(data{scan}, 'scan_type')
                    if strcmp(data{scan}.scan_type, 'HCD')
                        fragments{j}.validated = compare_spectra3(fragments{j}, data{scan}.scan_data(temp,:), HCD_tol);
                    else
                        fragments{j}.validated = compare_spectra3(fragments{j}, data{scan}.scan_data(temp,:), CID_tol);
                    end
                else
                    fragments{j}.validated = compare_spectra3(fragments{j}, data{scan}.scan_data(temp,:), CID_tol);
                end
        ...
        for num_id_peaks = 1:num_id_peaks_max
            ...
            if x > x_range(1) && x < x_range(2) % checks that mass is in range
                ...
                if ~isempty(name) % if peak assigned
                    if strcmp(name, 'isotope') % This might explain why we see isotopes before [MH]2+
...
% Write XLS file with iTRAQ data for scans in list
    function iTRAQ_to_Excel(excel_list)
        [iTRAQ_filename, iTRAQ_path] = uiputfile('*.xlsx','Save iTRAQ Summary',[OUT_path, filename, '.xlsx']);
        XLS_out = [iTRAQ_path iTRAQ_filename];
        
        out = horzcat({'Scan', 'Protein', 'Accession', 'Sequence'},num2cell(iTRAQ_masses));
        ...
        for i = 1:length(excel_list)
            ...
            out = vertcat(out,line);
        end
        xlswrite(XLS_out,out,'Accept')
    end
